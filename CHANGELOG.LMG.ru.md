...
Формат версии: `<upstream>-lmgN`. Текущий выпуск: **1.11.0-lmg31**.

## 1.11.0-lmg31

Перенесена Google 1.11.0. Кроссфейд адаптирован к RendererHolder, очередям по рендерерам и аудиофокусу в playback-потоке. Формулы кривых и публичный API сохранены. Ограничитель частоты обновлений использует монотонные часы ExoPlayer. Публикация перенесена на Kotlin DSL с единой версией. Собраны 11 AAR; 136 JVM-тестов прошли. Эмулятор и проверка звука на устройстве не запускались. [Подробности](MIGRATION-1.11.0.md).
